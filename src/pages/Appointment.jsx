...
import RelatedDoctors from "../components/RelatedDoctores";
...
const Appointment = () => {
  ...
  // fetch doctor info
  const fetchDocInfo = () => {
    if (!doctors.length) return; // wait until doctors are loaded
  ...
  // load doctor info when doctors or docId change
  useEffect(() => {
    fetchDocInfo();
  }, [doctors, docId]);

  // load slots when docInfo is available
  useEffect(() => {
